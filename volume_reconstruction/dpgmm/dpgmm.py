...
#      self.data = map(lambda x: x.copy(), dims.data)
      self.data = [x.copy() for x in dims.data]
...
#      self.n = map(lambda x: GaussianPrior(x), dims.n)
      self.n = [GaussianPrior(x) for x in dims.n]
      ...
      self.nT = [None if x==None else StudentT(x) for x in dims.nT]
      ...
      self.priorT = None
#      self.n = map(lambda _: GaussianPrior(self.dims), range(self.stickCap)) # The mixture components, one for each stick.
      self.n = [GaussianPrior(self.dims) for _ in range(self.stickCap)] # The mixture components, one for each stick.
  ...
  def incStickCap(self, inc = 1):
    ...
    self.stickCap += inc
    self.n += [GaussianPrior(self.dims) for _ in range(inc)]#map(lambda _: GaussianPrior(self.dims), range(inc))
  ...
  def setPrior(self, mean = None, covar = None, weight = None, scale = 1.0):
    ...
    if mean is None or covar is None:
      ...
      dm = self.getDM()
      for i in range(dm.shape[0]): inc.add(dm[i,:])
      ggd = inc.fetch()
  ...
  def solve(self, iterCap=None):
    ...
    prev = self.z.copy()
    iters = 0
    sys.stderr.write("components: %2d\n"%(self.stickCap))
    while True:
      iters += 1
  ...
  def solveGrow(self, iterCap=None, maxStick=None):
    ...
    prev = None
    while True:
      sys.stderr.write("number of sticks:"+str(self.getStickCap())+"\n")
      it += self.solve(iterCap)
      value = self.nllData()
      sys.stderr.write("logL(data):"+str(value)+"\n")
      if maxStick!=None: 
